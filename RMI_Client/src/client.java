	...
	public static void main(String args[]) {
		...
		if (args[0].equals("RMI")) {
			...
			if (args.length == 3) {
				...
				System.out
						.println("Usage: Usage: java client [rmihost [rmiport]]");
				System.exit(1);
			}
			...
			try {
				...
				rm = (ResourceManager) registry.lookup(serverName);
				if (rm != null) {
			...
			rm = SocRequester.newRMInstance(args[1]);
			System.out.println("Successful connected to Middleware server");
			// rm = SMIRequestor.newRMInstance(args[1]);
			// System.out.println("Successful connected to RM Master");
		} else {
			System.err
		...
		while (true) {
			...
			case 23: // start
				...
				try {
					int tid = rm.start();
					System.out.println("Transaction created ID: " + tid);
			...
			case 26: // shutdown
				...
				try {
					if (rm.shutdown()) {
						System.out.println("Resource Manager is shutdown.");
					} else {
						System.out.println("Resource Manager is not shutdown.");
					}
				} catch (RemoteException e) {
					// e.printStackTrace();
				}
