...
public class LockManager implements Serializable {
	...
	public static TPHashTable lockTable = new TPHashTable(
			LockManager.TABLE_SIZE);
	public static TPHashTable stampTable = new TPHashTable(
			LockManager.TABLE_SIZE);
	public static TPHashTable waitTable = new TPHashTable(
			LockManager.TABLE_SIZE);
				...
				synchronized (this.lockTable) {
					...
					if (!bConflict) {
						...
						if (bConvert.get(0) == true) {
							...
							TrxnObj newTrxnObj = (TrxnObj) this.lockTable
									.get(trxnObj);
							newTrxnObj.setLockType(TrxnObj.WRITE);
							dataObj.setLockType(DataObj.READ);
							DataObj newDataObj = (DataObj) this.lockTable
									.get(dataObj);
							newDataObj.setLockType(DataObj.WRITE);
						} else {
				...
				if (dataObj.getLockType() == DataObj.READ) {
					...
					if (dataObj2.getLockType() == DataObj.READ) {
						...
						throw new RedundantLockRequestException(
								dataObj.getXId(), "Redundant READ lock request");
					}
				}
			} else {
				if (dataObj.getLockType() == DataObj.READ) {
